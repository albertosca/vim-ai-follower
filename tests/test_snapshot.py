...
def test_different_sessions_do_not_collide(tmp_path: Path) -> None:
    snapshot.save("$1", "/tmp/a.txt", "from session 1", base_dir=tmp_path)
    snapshot.save("$2", "/tmp/a.txt", "from session 2", base_dir=tmp_path)
    assert snapshot.load("$1", "/tmp/a.txt", base_dir=tmp_path) == "from session 1"
    assert snapshot.load("$2", "/tmp/a.txt", base_dir=tmp_path) == "from session 2"
